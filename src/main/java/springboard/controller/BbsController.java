...
import springboard.commnd.BbsCommandImpl;
import springboard.commnd.DeleteActionCommand;
import springboard.commnd.EditActionCommand;
import springboard.commnd.EditCommand;
import springboard.commnd.ListCommand;
import springboard.commnd.ReplyActionCommand;
import springboard.commnd.ReplyCommand;
import springboard.commnd.ViewCommand;
import springboard.commnd.WriteActionCommand;
...
public class BbsController {
	...
	/*
		@Autowired
			...
			싶을때 사용한다. 타입을 기반으로 자동주입되며, 만약 해당
			타입의 빈이 존재하지 않으면 에러가 발생되어 서버를 시작할
			수 없다.
			- 생성자, 멤버변수, 메소드(setter)에 적용가능함.
			- 타입을 이용해 자동으로 프로퍼티 값을 설정함.
			- 해당 어노테이션은 멤버변수만 적용할 수 있다. 함수내의
				지역변수에서는 사용할 수 없다.
			- 타입을 통해 자동으로 설정되므로 같은 타입의 2개이상
				존재하면 예외가 발생됨.
	 */
	...
	/*
		멤버변수로 선언하여 클래스에서 전엮적으로 사용할 수 있다. 해당
